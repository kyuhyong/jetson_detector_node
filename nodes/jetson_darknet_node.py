...
class JetsonDarknetNode:
    ...
    def __init__(self):
        ...
        # Get relay parameters from launch file
        self.relay_on_delay = rospy.get_param('~relay_on_delay_ms', '1000') / 100
        rospy.loginfo("[GET PARAM] Relay wait time: %d /10 seconds", self.relay_on_delay)
        self.relay_off_delay = rospy.get_param('~relay_off_delay_ms', '1500') / 100
        rospy.loginfo("[GET PARAM] Relay on time: %d /10 seconds",self.relay_off_delay)
                ...
                if self.box.time_last == 0:
                    ...
                    if self.timer1 - self.box.time_last < 25:
                        ...
                        if self.box.count > 1:
                            ...
                            #rospy.loginfo("X: {}, Y: {} Width: {}, Height: {}".format(box.xmin, box.ymin, width, height))
        if OLED:
